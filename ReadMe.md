...
Project within `occt-gtk-glarea` subfolder shows OCCT 3D viewer setup
from *OpenGL* context created by `Gtk::GLArea` within GTK application.

## Troubleshooting

The sample relies on X11 backend (`GDK_BACKEND=x11`), so that on Wayland session it will be running through Xwayland (which is expected to be done automatically).
